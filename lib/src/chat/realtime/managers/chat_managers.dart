...
class PrivateChatManager extends BaseChatManager<PrivateChat> implements StreamedManager {
  ...
  StreamController<PrivateChat> _chatListStreamController = StreamController.broadcast();

  Stream<PrivateChat> get chatListStream => _chatListStreamController.stream;

  late ChatManager _chatManager;

  PrivateChatManager._private(Connection connection) : super._private(connection) {
  ...
  @override
  Future<CubeMessage> sendMessageInternal(CubeMessage message, PrivateChat chat) {
  ...
  PrivateChat createChat(int participantId) {
    Chat chat = _chatManager.getChat(Jid.fromFullJid(getJidForUser(participantId)));
  ...
  void chatListUpdated(List<Chat> newChatsList) {
    for (Chat chat in newChatsList) {
      ...
      if (!_chatsMap.containsKey(userId)) {
        ...
        chat.newMessageStream.listen((event) => processIncomingPacket(event.messageStanza));
      }
    }
  }
...
class GroupChatManager extends BaseChatManager<GroupChat> implements StreamedManager {
  ...
  StreamController<GroupChat> _chatListStreamController = StreamController.broadcast();

  Stream<GroupChat> get chatListStream => _chatListStreamController.stream;

  GroupChatManager._private(Connection connection) : super._private(connection) {
  ...
  Future<CubeMessage> sendMessageInternal(CubeMessage message, GroupChat chat) {
    ...
    CubeChatConnection.instance.asyncStanzaSender?.sendAsync(stanza, (resultStanza, exception) {
  ...
  Future<GroupChat> join(GroupChat groupChat) {
    ...
    JoinPresenceStanza joinStanza = JoinPresenceStanza(groupChat._dialogId!, CubeChatConnection.instance.currentUser!.id!);

    CubeChatConnection.instance.asyncStanzaSender?.sendAsync(joinStanza, (resultStanza, exception) {
      if (resultStanza == null) {
        completer.completeError(exception);
      } else {
  ...
  Future<GroupChat> leave(GroupChat groupChat) {
    ...
    LeavePresenceStanza leaveStanza = LeavePresenceStanza(groupChat._dialogId!, CubeChatConnection.instance.currentUser!.id!);

    CubeChatConnection.instance.asyncStanzaSender?.sendAsync(leaveStanza, (resultStanza, exception) {
      if (resultStanza == null) {
        completer.completeError(exception);
      } else {
        completer.complete(groupChat);
...
abstract class BaseChatManager<CH extends AbstractChat> extends Manager {
  ...
  Future<void> sendEditMessageStatus(CubeMessage originalMessage, bool isLast, Jid toJid, MessageStanzaType chatType) {
    Completer completer = Completer();
    ...
    if (isEmpty(body) || isEmpty(messageId) || isEmpty(dialogId)) {
      completer.completeError(
          IllegalArgumentException("To edit message original message should contains updated body, 'messageId' and 'dialogId'"));
      return completer.future;
    }
  ...
  Future<void> sendDeleteMessageStatus(CubeMessage originalMessage, Jid toJid, MessageStanzaType chatType) {
    Completer completer = Completer();
    ...
    if (isEmpty(messageId) || isEmpty(dialogId)) {
      completer.completeError(IllegalArgumentException("To delete message original message should contains 'messageId' and 'dialogId'"));
      return completer.future;
    }
...
class PrivateChat extends BaseCubeChat {
  ...
  void processIncomingMessage(MessageStanza messageStanza) {
    if (MessageStanzaType.ERROR == messageStanza.type) {
      _notifyErrorMessage(this, ErrorPacket.fromStanza(messageStanza.getChild('error')!));
  ...
  Future<void> deleteMessage(CubeMessage message) {
    return _privateChatManager.sendDeleteMessageStatus(message, Jid.fromFullJid(getJidForUser(_participantId)), MessageStanzaType.CHAT);
  }

  @override
  Future<void> editMessage(CubeMessage message, bool isLast) {
    return _privateChatManager.sendEditMessageStatus(
        message, isLast, Jid.fromFullJid(getJidForUser(_participantId)), MessageStanzaType.CHAT);
  }
}
...
class GroupChat extends BaseCubeChat {
  ...
  @override
  Future<void> deleteMessage(CubeMessage message) {
    return _groupChatManager.sendDeleteMessageStatus(message, Jid.fromFullJid(getJidForGroupChat(_dialogId!)), MessageStanzaType.GROUPCHAT);
  }

  @override
  Future<void> editMessage(CubeMessage message, bool isLast) {
    return _groupChatManager.sendEditMessageStatus(
        message, isLast, Jid.fromFullJid(getJidForGroupChat(_dialogId!)), MessageStanzaType.GROUPCHAT);
  }
}

abstract class BaseCubeChat implements AbstractChat {
  StreamController<CubeMessage> _chatMessagesStreamController = StreamController.broadcast();
...
Future<void> updateMessageStatus(Connection connection, String status, CubeMessage originalMessage) {
  Completer completer = Completer();
  ...
  if (senderId == null || senderId == 0 || isEmpty(messageId) || isEmpty(dialogId)) {
    completer.completeError(
        IllegalArgumentException("To update message status original message should contains 'senderId', 'messageId', 'dialogId'"));
    return completer.future;
  }
  ...
  CubeMessageStanza statusMessage = CubeMessageStanza(stanzaId, MessageStanzaType.CHAT);
...
Future<void> sendDeliveredStatus(Connection connection, CubeMessage cubeMessage) {
