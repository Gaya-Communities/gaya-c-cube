...
class ConferenceSession
  ...
  Future<void> joinDialog(String dialogId, callback(List<int?> publishers),
      {ConferenceRole conferenceRole = ConferenceRole.PUBLISHER}) {
    _joinCallback = callback;
    sessionDescription.conferenceRole = conferenceRole;
    return _signaler!.joinDialog(
        dialogId, client.currentUserId, JOIN_JANUS_ROLE, conferenceRole);
  }
  ...
  Future<void> subscribeToPublisher(int? publisherId) async {
    ...
    try {
      await _signaler!
          .attachPlugin(ConferenceConfig.instance.plugin, publisherId);
  ...
  void performSendOffer() {
    ...
    _makeAndAddNewChannelForOpponent(
        currentUserId, sessionDescription.conferenceType);
    createOffer(currentUserId);
  }

  void proceedAsListener() {
    setState(RTCSessionState.RTC_SESSION_CONNECTING);
  }
  ...
  void _makeAndAddNewChannelForOpponent(int opponentId, int conferenceType) {
    if (!channels.containsKey(opponentId)) {
      ConferencePeerConnection newChannel =
          new ConferencePeerConnection(opponentId, this);
...
//    }
  ...
  void onPeerConnectionStateChanged(int userId, PeerConnectionState state) {
    switch (state) {
      case PeerConnectionState.RTC_CONNECTION_CONNECTED:
        ...
        logTime(
            "onPeerConnectionStateChanged can send sendIceCandidateComplete()",
            _TAG);
        break;
      default:
        super.onPeerConnectionStateChanged(userId, state);
  ...
  void onSendAnswer(int userId, RTCSessionDescription sdp) async {
    ...
    try {
      await _signaler!
          .sendAnswer(userId, sdp, sessionDescription.conferenceType);
    } on WsException catch (ex) {
  ...
  void onSendOffer(int userId, RTCSessionDescription sdp) async {
    ...
    try {
      await _signaler!
          .sendOffer(userId, sdp, sessionDescription.conferenceType);
    } on WsException catch (ex) {
