...
//    socketConnection.send(packet);
  ...
  Future<void> joinDialog(String dialogId, int? userId, String janusRole, ConferenceRole conferenceRole) {
    this._userId = userId;
  ...
  Future<void> sendOffer(
      int? userId, RTCSessionDescription rtcSdp, int callType) {
  ...
  void packetParser(WsPacket packet) {
    if (packet is WsEvent) {
      ...
      _janusResponseCallback!
          .onEventError(packet.error.reason, packet.error.code);
    }
  }
  ...
  void eventParser(WsEvent wsEvent) {
    if (wsEvent.isRemoteSDPEventAnswer()) {
      ...
      if (opponentId != null) {
        _janusResponseCallback!
            .onRemoteSDPEventOffer(opponentId, wsEvent.jsep!.sdp);
      }
      ...
      if (displayRole != null) {
        conferenceRole = ConferenceRole.values
            .firstWhere((e) => e.toString() == 'ConferenceRole.' + displayRole);
      }
      logTime(
      ...
      _janusResponseCallback!
          .onJoinEvent(publishersList, subscribersList, wsEvent.sender);
