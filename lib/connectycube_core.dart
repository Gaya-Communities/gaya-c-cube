...
init(String applicationId, String authorizationKey, String authorizationSecret, {Future<CubeSession> Function()? onSessionRestore}) {
  CubeSettings.instance.init(applicationId, authorizationKey, authorizationSecret, onSessionRestore: onSessionRestore);
}
...
Future<CubeSession> createSessionUsingSocialProvider(String socialProvider, String accessToken, [String? accessTokenSecret]) {
  return CreateSessionQuery.usingSocial(socialProvider, List.of({accessToken, accessTokenSecret})).perform();
}

Future<CubeSession> createSessionUsingFirebase(String projectId, String accessToken) {
  return CreateSessionQuery.usingSocial(CubeProvider.FIREBASE_PHONE, List.of({projectId, accessToken})).perform();
}
...
Future<CubeUser> signInUsingSocialProvider(String socialProvider, String accessToken, [String? accessTokenSecret]) {
  return SignInQuery.usingSocial(socialProvider, List.of({accessToken, accessTokenSecret})).perform();
}

Future<CubeUser> signInUsingFirebase(String projectId, String accessToken) {
  return SignInQuery.usingSocial(CubeProvider.FIREBASE_PHONE, List.of({projectId, accessToken})).perform();
}
...
Future<PagedResult<CubeUser>?> getAllUsersByIds(Set<int> ids) {
  return GetUsersQuery.byFilter(RequestFilter(RequestFieldType.NUMBER, "id", QueryRule.IN, ids.join(","))).perform();
}
...
Future<CubeUser?> getUserByIdentifier(String identifierName, String? identifierValue) {
...
Future<AddressBookResult?> uploadAddressBook(List<CubeContact> contacts, {bool? force, String? udid}) {
...
Future<List<CubeUser>?> getRegisteredUsersFromAddressBook(bool compact, [String? udid]) {
